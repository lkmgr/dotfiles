...
client.connect_signal("request::default_keybindings", function()
    awful.keyboard.append_client_keybindings({
        ...
        awful.key({ mod.super, mod.ctrl, mod.alt }, "space", awful.client.floating.toggle,
                  { description = "toggle floating", group = "client" }),
        awful.key({ mod.super, "Control" }, "Return",
                  function(c) c:swap(awful.client.getmaster()) end,
                  { description = "move to master", group = "client" }),
        -- awful.key({ mod.super }, "o", function (c) c:move_to_screen() end,
        --         {description = "move to screen", group = "client"}),
        awful.key({ mod.super, "Control", "Shift" }, "k",
                  function(c) c:move_to_screen(c.screen.index - 1) end,
                  { description = "move to previous screen", group = "screen" }),
        awful.key({ mod.super, "Control", "Shift" }, "j",
                  function(c) c:move_to_screen(c.screen.index + 1) end,
                  { description = "move to next screen", group = "screen" }),
        awful.key({ mod.super }, "t", function(c) c.ontop = not c.ontop end,
                  { description = "toggle keep on top", group = "client" }),
        awful.key({ mod.super }, "Up", function() awful.client.incwfact(-0.02) end,
                  { description = "decrease client wfact", group = "layout" }),
        awful.key({ mod.super }, "Down", function() awful.client.incwfact(0.02) end,
                  { description = "increase client wfact", group = "layout" }),
